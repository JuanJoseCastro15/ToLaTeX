...
def detectarBordesYEsquinas(original, gris):
    ...
    # Convertir la imagen gris suavizada a float32, que es lo que espera cornerHarris.
    # El detector de Harris se aplica directamente sobre la imagen en escala de grises,
    # no sobre el resultado del detector de bordes Canny.
    esquina_input = np.float32(filtroEsquina)

    # blockSize: Tamaño de la vecindad para la detección de esquinas.
    # ksize: Apertura del operador Sobel utilizado.
    # k: Parámetro de sensibilidad del detector Harris.
    esquina = cv2.cornerHarris(esquina_input, 14, 5, 0.05) # Se aumentó 'k' para hacer el detector más selectivo
    esquina = cv2.dilate(esquina, None)

    # Ajustar el umbral para detectar más o menos esquinas.
    umbral = 0.20 * esquina.max() # Se incrementa el umbral para reducir más falsos positivos
...
if __name__ == "__main__":
    # Especifica directamente la ruta de la imagen aquí.
    # Por ejemplo, '/content/h2.png' es una imagen de ejemplo que tienes en Colab.
    # Cambia esta ruta si quieres probar con otra imagen.
    rutaImagen = "h2.png"
